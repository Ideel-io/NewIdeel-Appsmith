export default {
	...
	async addHousing() {
		try{
			...
			showModal(RelanceScraperAuto.name);
		} catch(error){
	...
	getDefaultHousingSelectionValue() {
		if (utils.getAnswer(getProcedureDetails.data.data.getProcedureDetails, "housing_selection_vehicle")) {
			return utils.getAnswer(getProcedureDetails.data.data.getProcedureDetails, "housing_selection_vehicle");
		}
		
		if (!getLastAutoRunData.data.data.getLastAutoRunData.userPersonalInfo.route && getUserAllHousingsByStaff.data && getUserAllHousingsByStaff.data.data.getUserAllHousingsByStaff.housings.length === 1) {
