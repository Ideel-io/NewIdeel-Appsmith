export default {
	...
	getDefaultProfileSelectionValue() {
		...
		if (getLastHealthRunData.data.data.getLastHealthRunData.userHealthInfo.profile_selection_credit_health) {
			return getLastHealthRunData.data.data.getLastHealthRunData.userHealthInfo.profile_selection_credit_health;
		}
	...
	async handleIframeMessage() {
		...
		if (message.type === "familyMemberAdded" || message.type === "familyMemberUpdated")  {
			await showModal(RelanceScraperHealth.name);
