...
async uploadIdentityVerso() {
  ...
  try {
    ...
    await getDocuments.run();
		...
		const documentKeys = getDocuments.data.data?.previousContract?.map((doc) => doc.documentKey) ?? [];
		
		await attachDocumentsToSubscription.run({
			userId: appsmith.URL.queryParams.userId, 
	...
	async uplaodDrivingLicense() {
		
		const files = Permis_de_conduire_File_Picker.files;
  if (!files || files.length === 0) {
    showAlert('Please select files to upload', 'warning');
    return;
  }
  ...
  try {
    ...
    await getDocuments.run();
		...
		const documentKeys = getDocuments.data.data?.drivingLicense?.map((doc) => doc.documentKey) ?? [];
	...
	async uplaodGrayCard() {
		
		const files = Carte_grise_File_Picker.files;
  if (!files || files.length === 0) {
    showAlert('Please select files to upload', 'warning');
    return;
  }
  ...
  try {
    ...
    await getDocuments.run();
		...
		const documentKeys = getDocuments.data.data?.grayCard?.map((doc) => doc.documentKey) ?? [];
	...
	async uplaodInformationStatement() {	
		
		const files = Releve_info_File_Picker.files;
  if (!files || files.length === 0) {
    showAlert('Please select files to upload', 'warning');
    return;
  }
  ...
  try {
    ...
    await getDocuments.run();
		...
		const documentKeys = getDocuments.data.data?.informationStatement?.map((doc) => doc.documentKey) ?? [];
	...
	async deleteDocument(label, fileName) {
		try{
			...
			await detachDocumentFromSubscription.run({
				userId: appsmith.URL.queryParams.userId, 
        recurrentSubscriptionId: appsmith.URL.queryParams.recurrentSubscriptionId, 
